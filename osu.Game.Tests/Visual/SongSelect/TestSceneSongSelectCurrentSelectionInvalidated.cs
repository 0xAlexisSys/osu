...
namespace osu.Game.Tests.Visual.SongSelect
{
    ...
    public partial class TestSceneSongSelectCurrentSelectionInvalidated : SongSelectTestScene
    {
        ...
        public void TestRulesetChange()
        {
            ...
            AddAssert("carousel beatmap is taiko", () => (initiallySelected = selectedBeatmap)?.Ruleset.ID, () => Is.EqualTo(1));
            AddUntilStep("global beatmap is taiko", () => Beatmap.Value.BeatmapInfo.Ruleset.ID, () => Is.EqualTo(1));

            ChangeRuleset(0);
            waitForFiltering(7);
            AddAssert("carousel beatmap is osu", () => selectedBeatmap?.Ruleset.ID, () => Is.EqualTo(0));
            AddUntilStep("global beatmap is osu", () => Beatmap.Value.BeatmapInfo.Ruleset.ID, () => Is.EqualTo(0));
            AddAssert("carousel beatmap is same set as original", () => selectedBeatmap?.BeatmapSet, () => Is.EqualTo(initiallySelected!.BeatmapSet));

            ChangeRuleset(1);
            waitForFiltering(8);
            AddAssert("carousel beatmap is taiko", () => selectedBeatmap?.Ruleset.ID, () => Is.EqualTo(1));
            AddUntilStep("global beatmap is taiko", () => Beatmap.Value.BeatmapInfo.Ruleset.ID, () => Is.EqualTo(1));
            AddAssert("carousel beatmap is same set as original", () => selectedBeatmap?.BeatmapSet, () => Is.EqualTo(initiallySelected!.BeatmapSet));

            ChangeRuleset(2);
            waitForFiltering(9);
            AddAssert("carousel beatmap is catch", () => selectedBeatmap?.Ruleset.ID, () => Is.EqualTo(2));
            AddUntilStep("global beatmap is catch", () => Beatmap.Value.BeatmapInfo.Ruleset.ID, () => Is.EqualTo(2));
